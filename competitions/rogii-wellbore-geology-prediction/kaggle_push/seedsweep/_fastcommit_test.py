...
SWEEP_SEEDS = 30  # growing seed-ensemble knob (target for scoring); auto-forced to 1 on preview
import glob as _pg
_prev_files = _pg.glob('/kaggle/input/**/test/*__horizontal_well.csv', recursive=True) or _pg.glob('**/test/*__horizontal_well.csv', recursive=True)
PREVIEW_SKIP = (len(_prev_files) == 3)  # local commit preview has exactly 3 test wells; real scoring test differs
if PREVIEW_SKIP:
    SWEEP_SEEDS = 1
    print(f"[FAST-COMMIT] preview ({len(_prev_files)} test wells) -> SWEEP_SEEDS=1 + tiny knobs + train subsample; full pipeline runs at scoring")
...
LEARNED_MODEL_ROOTS = ()  # DETACHED -> seed-driven train-from-scratch
...
if globals().get('PREVIEW_SKIP', False):  # FAST-COMMIT c7
    SP45_SELECTOR_N_PARTICLES = 16
    VISIBLE_PREFIX_PARTICLES = 16
...
if globals().get("PREVIEW_SKIP", False):  # FAST-COMMIT c16 train subsample
    _kw = train_df["well"].drop_duplicates().head(12).tolist()
    train_df = train_df[train_df["well"].isin(_kw)].reset_index(drop=True)
    print(f"[FAST-COMMIT] train subsampled to {train_df['well'].nunique()} wells / {len(train_df)} rows")
...
class CFG:
    ...
    PF_PARTICLES = 500
    if globals().get("PREVIEW_SKIP", False):  # FAST-COMMIT c34
        PF_PARTICLES = 16
        n_splits = 3
    PF_SCALES = (3., 5., 8., 12.)
...
if globals().get('PREVIEW_SKIP', False):  # FAST-COMMIT c51
    _GOLD_PARTICLES = 16
