"""Comp-level runner: relnoise per-well noise PF (system-identification, NO sweep) → blend → field-CV.
...
t0 = time.time()
out = "results/pf_relnoise.csv"
tb.trackB_oof("input/train", out, training=True, n_particles=500, n_seeds=128, scale=5.0, gpu=True, noise_id=True, noise_mode="relative")
print("relnoise PF done", round(time.time()-t0), flush=True)
...
feats.to_parquet("results/honest_feat_relnoise.parquet")
...
print("relnoise PF field-CV:", rmse(pf_d))
best = None
for w in np.linspace(0, 1, 21):
    r = rmse(w*pf_d + (1-w)*gbm)
    if best is None or r < best[0]: best = (r, round(float(w),2))
print("relnoise blend field-CV:", best, " (gs130 blend 10.56, blend_AB 10.75)")
