> **Source:** `source.pdf` · 36 pages · 6 figures · 150 display equations · 2 tables · converted by fleet `paper-md` (backend=**docling+pymupdf-assets**)
...
$$\min _ { b \in \mathbb { R } ^ { N } } \ \mathcal { J } ( b ) \triangle q \sum _ { i = 1 } ^ { N } w _ { i } \cdot D ( b _ { i } ) \ \ s . t . \quad \sum _ { i = 1 } ^ { N } b _ { i } = B , \ \ b _ { \min } \leq b _ { i } \leq b _ { \max }  \tag{1}$$
...
![Figure 2: RATEQUANT pipeline. Phases 1–3 are one-time offline costs (<2 s for 8B](assets/fig/p02_0.png)

*Figure 2: RATEQUANT pipeline. Phases 1–3 are one-time offline costs (<2 s for 8B); Phase 4 adds zero runtime overhead.*

![Figure 4: Per-head sensitivity for Qwen3-8B (36 layers × 8 KV heads, log scale).](assets/fig/p08_0.png)

*Figure 4: Per-head sensitivity for Qwen3-8B (36 layers × 8 KV heads, log scale). Left: Gradient-based shows a U-shaped pattern (early + late layers sensitive). Right: Activation-based is monotonically increasing.*

![Figure 5: PPL vs. bits (Qwen3-8B). Gap is largest at 2.5–3.5 bits.](assets/fig/p08_1.png)

*Figure 5: PPL vs. bits (Qwen3-8B). Gap is largest at 2.5–3.5 bits.*

![Figure 6: KIVI per-layer MSE at different bit-widths. Left: Key cache has high d](assets/fig/p17_0.png)

*Figure 6: KIVI per-layer MSE at different bit-widths. Left: Key cache has high distortion with strong per-layer variation. Right: Value cache has ∼4× lower MSE, driving the optimal K/V split (2.85/2.15 at 2.5 bits).*

![Figure 8: Per-head gradient sensitivity for Qwen3-8B (left: Key, right: Value). ](assets/fig/p22_0.png)

*Figure 8: Per-head gradient sensitivity for Qwen3-8B (left: Key, right: Value). Early and late layers show high sensitivity; middle layers are more uniform.*

![Table 19: Head sensitivity heterogeneity across models (gradient proxy).](assets/fig/p22_1.png)

*Table 19: Head sensitivity heterogeneity across models (gradient proxy).*

### Equation crops — every numbered formula as rendered (13)

**(1)** p3 ![eq](assets/eq/eq_p03_016.png)

**(3)** p4 ![eq](assets/eq/eq_p04_025.png)

**(4)** p14 ![eq](assets/eq/eq_p14_072.png)

**(5)** p26 ![eq](assets/eq/eq_p26_101.png)

**(6)** p26 ![eq](assets/eq/eq_p26_106.png)

**(7)** p26 ![eq](assets/eq/eq_p26_110.png)

**(9)** p26 ![eq](assets/eq/eq_p26_115.png)

**(10)** p26 ![eq](assets/eq/eq_p26_120.png)

**(12)** p27 ![eq](assets/eq/eq_p27_130.png)

**(13)** p27 ![eq](assets/eq/eq_p27_139.png)

**(14)** p27 ![eq](assets/eq/eq_p27_146.png)

**(15)** p27 ![eq](assets/eq/eq_p27_147.png)

**(16)** p27 ![eq](assets/eq/eq_p27_148.png)
