...
def compute_loss(logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    ...
    weight[div_rows] = float(os.environ.get("DIV_W", "1.0"))  # division-emphasis (target the +0.1)
...
class FrameWindowData:
    ...
    stride: int = 1                     # frame gap (frame-drop aug): frames are t_start, t_start+stride, ...
    frame_indices: tuple[int, ...] = () # explicit frame indices to read (non-contiguous for stride>1)
...
def compute_gap_transition_matrix(
    gt_ids_t: np.ndarray,
    gt_ids_t1: np.ndarray,
    children_map: dict[int, list[int]],
    stride: int,
) -> torch.Tensor:
    """GT transition matrix across a frame gap of ``stride`` (frame-drop aug).

    A node u@t is linked to w@(t+stride) iff w is a descendant of u exactly
    ``stride`` generations later (GT edges are consecutive-frame only, so
    following ``stride`` edges from frame t lands on frame t+stride). Divisions
    fork correctly: a parent reaches all its grand-descendants (multiple 1s per
    row); each descendant has exactly one ancestor at t (one 1 per column), so
    the linker's column-softmax assumption still holds.
    """
    col_of = {int(nid): j for j, nid in enumerate(gt_ids_t1)}
    matrix = torch.zeros(len(gt_ids_t), len(gt_ids_t1), dtype=torch.float32)
    for i, u in enumerate(gt_ids_t):
        reached = {int(u)}
        for _ in range(stride):
            nxt: set[int] = set()
            for n in reached:
                nxt.update(children_map.get(n, ()))
            reached = nxt
            if not reached:
                break
        for w in reached:
...
def get_gap_window_data(
    ...
    """Build a 2-frame gap pair (t_start, t_start+stride) with a composed target.

    Mirrors ``get_window_data`` but for two *non-contiguous* frames, teaching the
    linker to match under the larger displacement that missed detections create.
    Returns ``None`` if either frame has zero GT nodes.
    """
    gt_attrs = gt_graph.node_attrs(attr_keys=["node_id", "t", "z", "y", "x"])
    ds = np.array(downsample, dtype=np.float32)

    frames = [t_start, t_start + stride]
    pos_feats: list[torch.Tensor] = []
    coords_list: list[torch.Tensor] = []
    node_counts: list[int] = []
    per_frame_ids: list[np.ndarray] = []
    ...
    target = compute_gap_transition_matrix(
        per_frame_ids[0], per_frame_ids[1], children_map, stride)
    ...
    return FrameWindowData(
        ...
        targets=[target],
        stride=stride,
        frame_indices=(t_start, t_start + stride),
    )
...
def pad_window(
    ...
    fi = window.frame_indices or tuple(range(window.t_start, window.t_start + W))
    return {
        ...
        "frame_indices": torch.tensor(fi, dtype=torch.long),
    }
...
class FrameWindowDataset(Dataset):
    ...
    def __init__(
        ...
        # Per-item frame-drop stride, for the stride-grouped batch sampler.
        self.item_strides = [int(meta.get("stride", 1)) for meta, _ in self._data]
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        # Read the window's explicit frame indices (non-contiguous for frame-drop
        # gap pairs) with spatial downsample at I/O time → (W, Z_ds, Y_ds, X_ds).
        fi = meta["frame_indices"].tolist() if "frame_indices" in meta \
            else list(range(t_start, t_start + W))
        raw = np.stack([z[t, ::dz, ::dy, ::dx] for t in fi]).astype(np.float32)
...
class StrideGroupedBatchSampler(torch.utils.data.Sampler):
    """Yield batches whose items ALL share the same frame-drop stride.

    Frames are dropped uniformly across a batch (per the design), so a batch
    never mixes gaps. Indices are grouped by stride, shuffled within each group,
    chunked into batches, then the batch order is shuffled across strides.
    """

    def __init__(self, item_strides, batch_size, shuffle=True, generator=None):
        self.item_strides = list(item_strides)
    ...
    def __iter__(self):
        ...
        for idx, s in enumerate(self.item_strides):
            groups.setdefault(s, []).append(idx)
        batches: list[list[int]] = []
        for s, idxs in groups.items():
            idxs = list(idxs)
            if self.shuffle:
                perm = torch.randperm(len(idxs), generator=self.generator).tolist()
    ...
    def __len__(self) -> int:
        groups: dict[int, int] = {}
        for s in self.item_strides:
            groups[s] = groups.get(s, 0) + 1
        return sum((n + self.batch_size - 1) // self.batch_size for n in groups.values())
...
def load_dataset_windows(
    ...
    # Frame-drop augmentation: extra 2-frame gap pairs (t, t+stride) for stride>1,
    # with the GT link composed across the gap via lineage. Teaches the linker to
    # match under the larger displacement that missed detections create.
    gap_strides = [s for s in strides if s > 1]
    if gap_strides:
        edge_attrs = tracks.edge_attrs(attr_keys=["source_id", "target_id"])
        children_map: dict[int, list[int]] = {}
        for src, tgt in zip(edge_attrs["source_id"], edge_attrs["target_id"]):
            children_map.setdefault(int(src), []).append(int(tgt))
        # Optional deterministic decimation so frame-drop doesn't multiply the
        # dataset size N-fold (FRAME_STRIDE_FRAC in (0,1]; keep every k-th gap pair).
        frac = float(os.environ.get("FRAME_STRIDE_FRAC", "1.0"))
        keep_every = max(1, round(1.0 / frac)) if frac < 1.0 else 1
        for stride in gap_strides:
            for j, t in enumerate(range(image_shape[0] - stride)):
                if keep_every > 1 and (j % keep_every) != 0:
                    continue
                data = get_gap_window_data(
                    tracks, image_shape, t, stride, children_map, downsample=downsample)
                if data is not None:
                    windows.append(data)
...
def train_epoch(
    ...
    for _ in pbar:
        ...
        # Frame-drop soft confidence: a composed (t,t+n) link is a real lineage
        # edge but less certain the bigger the gap, so it contributes less.
        # conf = 1 - 0.05*(n-1)  →  n=1:1.0  n=2:0.95  n=3:0.90  n=4:0.85.
        # Whole batch shares one stride (stride-grouped sampler), so one scalar.
        if "stride" in batch:
            stride = int(batch["stride"][0].item())
            edge_loss = edge_loss * (1.0 - 0.05 * (stride - 1))
...
def train(
    ...
    # Frame-drop augmentation strides (env FRAME_STRIDES, e.g. "1,2,3,4"). Applied
    # to TRAIN only; test always uses consecutive pairs (stride 1).
    _frame_strides = tuple(
        int(s) for s in os.environ.get("FRAME_STRIDES", "1").split(",") if s.strip())
    if _frame_strides != (1,):
        print(f"Frame-drop augmentation: strides={_frame_strides} (train only)", flush=True)

    def _load(
        files: list[Path], desc: str, strides: tuple[int, ...] = (1,),
    ...
    train_video_data = _load(train_files, "train", strides=_frame_strides)
    test_video_data = _load(test_files, "test", strides=(1,))
    ...
    if _frame_strides != (1,):
        # Frame-drop: batches must be single-stride → custom batch sampler.
        batch_sampler = StrideGroupedBatchSampler(
            train_ds.item_strides, batch_size, shuffle=True, generator=g)
    ...
    # Full-model WARM-START (fine-tune from pretrained UNet+transformer+detect-head).
    _init_w = os.environ.get("INIT_WEIGHTS", "").strip()
    if _init_w:
        st = torch.load(_init_w, map_location=device, weights_only=True)
        miss, unexp = model.load_state_dict(st, strict=False)
        print(f"  WARM-START full model from {_init_w}: {len(miss)} missing, {len(unexp)} unexpected", flush=True)
    ...
    # Optionally FREEZE the UNet + detection head so only the linker (transformer)
    # retrains. Keeps detection (the 97.6%-recall dots) byte-identical and makes the
    # fine-tune small/fast — isolates the linking lever cleanly. Env-gated.
    if os.environ.get("FREEZE_UNET", "0") == "1":
        for p in model.unet.parameters():
            p.requires_grad = False
        for p in model.detect_head.parameters():
            p.requires_grad = False
        model.unet.eval()
        n_frozen = sum(p.numel() for p in model.unet.parameters()) + \
            sum(p.numel() for p in model.detect_head.parameters())
        print(f"  FROZEN UNet + detect_head ({n_frozen:,} params) — training linker only", flush=True)
