...
- If you need to report submission results, blockers, or completion to `leader`, prefer `python -m researchpapers.runtime_cli --config researchpapers/runtime.toml delegate --from trainer --to leader "..."` rather than hand-editing `thread.jsonl` as if it were a direct message.
...
- If formal backtesting has already been submitted to `researchpapers/train_service/`, treat yourself as "waiting for external results" rather than continuing to run long backtesting inside the tmux pane.
