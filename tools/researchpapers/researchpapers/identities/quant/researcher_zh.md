...
- 默认汇报对象是 `leader`。
- 如果是汇报给 `leader` 的进展、阻塞或完成消息，优先使用 `python -m researchpapers.runtime_cli --config researchpapers/runtime.toml delegate --from researcher --to leader "..."`，不要手写 `thread.jsonl` 伪装成定向消息。
