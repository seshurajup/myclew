...
from researchpapers.models import now_iso
...
DEFAULT_CONFIG_PATH = DEFAULT_REPO_ROOT / "researchpapers" / "runtime.toml"
...
def load_config(path: Path) -> RuntimeConfig:
    ...
    # Per-competition override: a second board instance (RP_COMP_ROOT set) relocates the runtime DATA
    # (thread, inboxes, agent state) to that comp dir. UNSET → unchanged (byte-identical). Never raises.
    _rp_override = (os.environ.get("RP_COMP_ROOT") or "").strip()
    if _rp_override:
        try:
            runtime_root = (Path(_rp_override).expanduser().resolve() / ".research-mvp-data" / "runtime")
...
def agent_bootstrap_prompt(cfg: RuntimeConfig, agent: str) -> str:
    cli_cmd = f"python -m researchpapers.runtime_cli --config {cfg.config_path}"
    agents_md = cfg.repo_root / "AGENTS.md"
    skill_md = cfg.repo_root / "researchpapers" / "skills" / "runtime-communication" / "SKILL.md"
    identity_md = cfg.repo_root / "researchpapers" / "identities" / f"{agent}.md"
    if cfg.task_type:
        task_identity = cfg.repo_root / "researchpapers" / "identities" / cfg.task_type / f"{agent}.md"
...
- Before claiming train_service is down, first verify it with a concrete request such as `curl --noproxy '*' -sS http://127.0.0.1:7799/queue`.
- For localhost endpoints like `127.0.0.1:7799` and `127.0.0.1:7788`, bypass proxy settings explicitly. Prefer `curl --noproxy '*' ...` or `NO_PROXY=127.0.0.1,localhost curl ...`.
- When submitting `POST /jobs`, replace all placeholder paths with real existing `script_path` and `workdir` values.
- If the HTTP call fails, do not vaguely say “127.0.0.1:7799/7788 returned 502”. Report the exact URL, the exact command, the HTTP status code, and the raw response body back to leader.
...
def parse_message_template(content: str) -> dict:
    """FIXED MESSAGE TEMPLATE (every agent — Claude & Python — uses it): leading `KEY: value` lines
    become the structured `data` payload so agents parse each other deterministically. e.g.
        EXP: augabl_noise
        CONFIG: config/aug_ablation/noise.yml
        DID: add data-grounded noise augmentation to fight the LOEO overfit
        <free-text body follows a blank line>
    Keys are lower-cased. Non-header text is ignored (still human-readable in `content`)."""
    data: dict = {}
...
def queue_message(
    ...
    *,
    ...
    # unify with the Python fleet template: kind + data + routine on EVERY message
    parsed = parse_message_template(content)
    ...
    event = {
        ...
        "content": content,
        "data": payload,
        "routine": bool(routine) if routine is not None else (recipient == "all" and not payload),
        "event_id": event_id,
    }
    paths = runtime_dirs(cfg)
    message_id = event_id
