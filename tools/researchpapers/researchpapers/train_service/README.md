...
`train_service` 是 `researchpapers` 内部的最小训练执行子服务。

它的目标不是做 LLM 编排，而是做三件事：

- 接收训练 shell 脚本任务
- 排队并执行一个个脚本
- 在任务完成后回调 `researchpapers`
...
- 完成后通过 HTTP 回调 `researchpapers`
- 自带单页看板 `/`

## Demo Scripts

- `researchpapers/train_service/demo_scripts/hello_world.sh`
- `researchpapers/train_service/demo_scripts/delay_echo.sh`
...
## 与 researchpapers 的关系

- `researchpapers`：智能编排层
...
- 回调 `researchpapers` 时只带 `train_task_id` 和原始请求元信息
