...
def read_log_tail(path: Path, *, lines: int = 120) -> list[str]:
    ...
    # drop the blank lines tqdm leaves behind after stripping its cursor-moves
...
async def get_job_log_alias(job_id: str, lines: int = 120) -> dict[str, Any]:
    # alias: the board frontend fetches /log (singular); real route is /logs
    return await get_job_logs(job_id, lines)
...
async def callback_researchpapers(payload: ResearchMVPCallbackRequest) -> JobActionResponse:
    ...
    append_job_event(payload.job_id, "researchpapers-style callback received")
