...
description: "Use this skill when working inside the researchpapers tmux runtime with fixed agents (leader, researcher, trainer) and you need to read shared thread messages, inspect per-agent inboxes, delegate tasks between agents, or follow the repository's runtime communication contract. Specifically for the file-backed runtime CLI under researchpapers/runtime_cli.py."
user-invocable: true
triggers:
...
Use this skill only for the local `researchpapers` runtime.

## Files To Read First

- `AGENTS.md`
- `researchpapers/runtime.toml`
...
python -m researchpapers.runtime_cli --config researchpapers/runtime.toml inbox list leader
```
...
```bash
python -m researchpapers.runtime_cli --config researchpapers/runtime.toml delegate --from leader --to researcher "..."
```
...
- If the recipient is specifically `leader`, prefer `python -m researchpapers.runtime_cli --config researchpapers/runtime.toml delegate --from <worker> --to leader "..."` so the message lands in both the thread and `leader` inbox.
