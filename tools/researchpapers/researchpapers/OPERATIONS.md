...
本文档描述 `researchpapers` runtime 的日常启动、停止、观察、排障、恢复和推荐操作流程。

## 1. 目标

`researchpapers` 的运维目标不是“部署一个 Web 服务”这么简单，而是保证以下三件事同时成立：
...
- `python -m researchpapers.runtime_cli up`：启动 tmux runtime，并默认进入控制环
- `train_service`：`researchpapers` 内部的训练队列与执行子服务，负责正式训练作业
- `python -m uvicorn researchpapers.app:app --port 8090`：Web 看板
...
```bash
python -m researchpapers.runtime_cli up
python -m uvicorn researchpapers.train_service.app:app --port 8100
python -m uvicorn researchpapers.app:app --port 8090
```

说明：

- `up` 负责启动或补齐 tmux runtime，并默认直接进入 `supervise`
- 如需只启动不进入控制环，可使用 `python -m researchpapers.runtime_cli up --no-supervise`
- `researchpapers/train_service/` 不属于 tmux runtime 生命周期的一部分，应作为单独 FastAPI 子服务启动和维护
...
```bash
python -m researchpapers.runtime_cli down
```
...
```bash
python -m researchpapers.runtime_cli status
```
...
```bash
python -m researchpapers.runtime_cli thread tail -n 50
```
...
```bash
python -m researchpapers.runtime_cli inbox list leader
python -m researchpapers.runtime_cli inbox list researcher
python -m researchpapers.runtime_cli inbox list trainer
```
...
```bash
python -m researchpapers.runtime_cli attach
```
...
```bash
python -m researchpapers.runtime_cli thread send --to leader "你的任务"
python -m researchpapers.runtime_cli thread tail -n 50
python -m researchpapers.runtime_cli inbox list leader
```
...
```bash
python -m researchpapers.runtime_cli thread tail -n 80
python -m researchpapers.runtime_cli status
python -m researchpapers.runtime_cli inbox list leader
python -m researchpapers.runtime_cli attach
```
...
```bash
python -m researchpapers.runtime_cli supervise
```
...
- 不要把正式训练长期挂在 `trainer` pane 中跑，正式训练应下沉到 `researchpapers/train_service/`
...
这些能力补齐后，`researchpapers` 会从“可操作原型”进一步变成“可维护 runtime”。
