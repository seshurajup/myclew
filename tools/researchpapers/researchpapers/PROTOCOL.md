...
本文档描述 `researchpapers` runtime 当前的通信协议、文件协议、任务协议和状态迁移规则。
...
- `researchpapers/train_service/` 负责维护正式训练 job 的详细状态
- `researchpapers` runtime 不直接托管长时间训练进程，只保留与训练 job 相关的任务、通知和结果摘要
- 当前仓库默认工作目录结构应为：
  - `recipe/`
  - `baseline/`
  - `src/baseline/`
...
- per-agent inbox
- workflow_state
- `researchpapers/train_service/` 的文件队列状态
...
当正式训练完成后，推荐由 `researchpapers/train_service/` 通过 runtime 协议向 `trainer` 发送一条消息：
...
- 如果某任务已经提交到 `researchpapers/train_service/` 并在正常排队或运行，不应继续按普通 stalled 处理
