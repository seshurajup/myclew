...
```bash
python -m researchpapers.runtime_cli init-config --force
```

Edit `researchpapers/runtime.toml` to set the working directory, runtime root, tmux session name, Codex command, per-agent env, and idle reminder thresholds. The default `codex_command` now starts Codex with `-m gpt-5.4`.
...
Use `python -m researchpapers.runtime_cli up --no-supervise` only if you explicitly want to skip the control loop.
...
```bash
python -m researchpapers.runtime_cli send leader "Review the thread and decide the next task."
```
...
`train_service` 现在作为 `researchpapers` 的子目录和子系统存在，位置是 `researchpapers/train_service/`。
...
- callback delivery to `researchpapers` at `/api/runtime/training-callback`
