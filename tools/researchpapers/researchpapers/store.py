...
from researchpapers.local_runtime import (
    MailboxManager,
    MessageType,
    TaskStatus,
    TeamManager,
...
from researchpapers.runtime_cli import DEFAULT_CODEX_COMMAND, DEFAULT_CONFIG_PATH, load_config

REPO_ROOT = Path(__file__).resolve().parent.parent


def _default_data_root() -> Path:
    """Per-competition DATA root: <RP_COMP_ROOT>/.research-mvp-data when set (a second board instance
    serving another comp), else this package's own root — byte-identical to the historical default.
    Never raises."""
    try:
        v = (os.environ.get("RP_COMP_ROOT") or "").strip()
        if v:
            return Path(v).expanduser().resolve() / ".research-mvp-data"
    except Exception:  # noqa: BLE001
        pass
    return REPO_ROOT / ".research-mvp-data"
...
class ProjectStore:
    ...
    def _spawn_leader_for_project(self, project: ProjectRecord) -> dict[str, str | bool]:
        ...
        spawn_message = backend.spawn(
            ...
            env={
                ...
                "RESEARCH_BOARD_URL": f"http://127.0.0.1:7788/projects/{project.id}",
    ...
    # kinds that carry real decision-signal for the leader/researcher (worker findings/verdicts/decisions).
    _SIGNAL_KINDS = {"finding", "verdict", "reason", "decision", "escalation"}
    _CORE_ROLES = {"human", "leader", "researcher", "trainer"}

    def _useful_for_leader(self, m: dict) -> bool:
        """The leader/researcher run on the (expensive) LLM — feed them ONLY purpose-useful messages,
        never the routine fleet chatter. Keep: non-routine messages that are (a) from/to a core role
        (human/leader/researcher/trainer), or (b) a real worker finding/verdict/decision/escalation.
        Drop everything else (routine status, low-signal 'note'/untyped worker broadcasts)."""
        if m.get("routine"):
            return False
        if (m.get("from") in self._CORE_ROLES) or (m.get("to") in ("leader", "researcher")):
            return True
        return m.get("kind") in self._SIGNAL_KINDS

    @staticmethod
    def _dedup_keep_newest(msgs: list[dict]) -> list[dict]:
        """Collapse near-identical content (same normalised first 120 chars) → keep the newest, so a
        repeated finding doesn't eat the leader's context budget. Preserves chronological order."""
        import re
        seen, keep = set(), []
        for m in reversed(msgs):                       # newest first
            sig = re.sub(r"\s+", " ", (m.get("content") or "").strip().lower())[:120]
            if sig in seen:
                continue
            seen.add(sig); keep.append(m)
        return list(reversed(keep))                    # back to chronological

    def _build_leader_prompt(self, project: ProjectRecord) -> str:
        # PURPOSE FILTER: only messages useful for reaching the goal reach the leader (not all chatter).
        useful = [m for m in self.project_messages(project.id) if self._useful_for_leader(m)]
        thread = self._dedup_keep_newest(useful)
        discussion = "\n".join(
            f"- [{msg.get('timestamp', '')}] {msg.get('from', '?')} -> {msg.get('to', 'leader')}: {msg.get('content', '')}"
            for msg in reversed(thread[-30:])
...
- Board URL: http://127.0.0.1:7788/projects/{project.id}
