...
def _default_runtime_root() -> Path:
    """Runtime control-plane dir. Priority: explicit RESEARCH_MVP_RUNTIME_DIR → RP_COMP_ROOT per-comp
    override → this package's own dir (byte-identical historical default). Never raises."""
    try:
        r = (os.environ.get("RESEARCH_MVP_RUNTIME_DIR") or "").strip()
        if r:
            return Path(r)
        c = (os.environ.get("RP_COMP_ROOT") or "").strip()
        if c:
            return Path(c).expanduser().resolve() / ".research-mvp-data" / "runtime"
    except Exception:  # noqa: BLE001
        pass
    return Path(__file__).resolve().parent.parent / ".research-mvp-data" / "runtime"


_ROOT = RuntimePaths(_default_runtime_root())
