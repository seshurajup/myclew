...
## The Python worker fleet — YOUR execution muscle (READ THIS)

You (leader/researcher) coordinate; **69 deterministic Python agents do the heavy lifting**. You are NOT
limited to delegating to each other — you dispatch real jobs to the fleet. Full catalog with one-line
purposes: **`FLEET_AGENTS.md`** (read it before planning; it lists every `kind`).

Dispatch a job (the matching worker claims it, runs it, scores via the official scorer, posts a finding):
```bash
python -m researchpapers.fleet_dispatch <kind> "<question>" '<optional spec JSON>'
python -m researchpapers.fleet_dispatch --list          # print the whole catalog anytime
```
Env for the command: `FLEET_COMPETITION_ROOT=<repo-root> PYTHONPATH=tools/researchpapers`.

High-value agents for the current goal (beat 0.897 on golden-12, node-recall / edge-Jaccard lever):
`fullconfig-search`, `config-ablate`, `perf-choice`, `combined-train`, `box-sample`, `sample-match`,
`scorer`, `reproduce-score`, `data-audit`, `xai`, `tracker-consensus`, `prior-art`, `trick-extractor`.

RULES: every recorded CV needs official-scorer JSON proof (the ledger REJECTS unproven winning scores).
Do NOT re-chase divisions (proven dead-end). NO Kaggle submit without escalating to the human.

### PREFER pipelines — think once, let the fleet run the whole chain

Do NOT reason between every step (that burns the LLM). Emit ONE `pipeline` job = an ordered list of
agents, and the fleet runs them one-by-one deterministically, carrying each result forward, with NO
Claude in the loop. Only the final summary comes back to you.

```bash
python -m researchpapers.fleet_dispatch pipeline "beat 0.897 via node-recall" '{"steps":[
  {"kind":"data-audit"},
...
```
The chain halts if a step escalates/fails (`stop_on_escalate`), carries scalar outputs forward (`carry`),
and surfaces any measured CV. Use single `fleet_dispatch <kind>` only for a one-off; use `pipeline` for
real work. Every recorded CV still needs official-scorer JSON proof.

### Pipeline templates (name one, the fleet runs the whole chain)
- `node_recall`   — data-audit → [fullconfig-search ∥ config-ablate] → scorer   (the 0.897 lever)
- `external_train`— data-audit → ext-label-stats → flow-gt-build → box-sample → sample-match → gnn-link-train → combined-train → scorer   (**external data + training, done right**)
- `full_e2e`      — external+train then [fullconfig-search ∥ config-ablate] → scorer → reproduce-score
- `config_search` — fullconfig-search → config-ablate → scorer   (no training)
- `diagnose`      — perf-choice → data-audit → eda-stats

Dispatch: `python -m researchpapers.fleet_dispatch pipeline "<goal>" '{"template":"external_train"}'`.
Add `"loop_until":{"metric":"best_cv","target":0.897,"max_rounds":3}` to iterate to the bar. Gate risky
steps with `"when":"best_cv>=0.897"` (e.g. submission-build) and `"on_fail":"skip"|"heal"`.
