"""pre_analysis_test — per-agent verifier: run `pre-analysis` on real repo state and assert it returns a valid
...
def test_pre_analysis_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test pre-analysis", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_pre_analysis_runs_and_returns_contract()
