"""guard_test — per-agent verifier: run `guard` on real repo state and assert it returns a valid
...
def test_guard_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test guard", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_guard_runs_and_returns_contract()
