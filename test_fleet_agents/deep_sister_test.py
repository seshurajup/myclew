"""deep_sister_test — per-agent verifier: run `deep-sister` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_deep_sister_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test deep-sister", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_deep_sister_runs_and_returns_contract()
