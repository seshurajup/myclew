"""baseline_test — per-agent verifier: run `baseline` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_baseline_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test baseline", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_baseline_runs_and_returns_contract()
