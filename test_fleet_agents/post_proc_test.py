"""post_proc_test — per-agent verifier: run `post-proc` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_post_proc_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test post-proc", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_post_proc_runs_and_returns_contract()
