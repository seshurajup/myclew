"""plan_ingest_test — per-agent verifier: run `plan-ingest` on real repo state and assert it returns a valid
...
def test_plan_ingest_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test plan-ingest", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_plan_ingest_runs_and_returns_contract()
