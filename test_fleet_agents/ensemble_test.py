"""ensemble_test — per-agent verifier: run `ensemble` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_ensemble_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test ensemble", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_ensemble_runs_and_returns_contract()
