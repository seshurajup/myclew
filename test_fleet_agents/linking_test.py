"""linking_test — per-agent verifier: run `linking` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_linking_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test linking", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_linking_runs_and_returns_contract()
