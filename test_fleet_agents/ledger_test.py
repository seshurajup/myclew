"""ledger_test — per-agent verifier: run `ledger` on real repo state and assert it returns a valid
...
def test_ledger_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test ledger", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_ledger_runs_and_returns_contract()
