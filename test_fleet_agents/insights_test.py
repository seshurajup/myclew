"""insights_test — per-agent verifier: run `insights` on real repo state and assert it returns a valid
...
def test_insights_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test insights", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_insights_runs_and_returns_contract()
