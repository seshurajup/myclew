"""learn_test — per-agent verifier: run `learn` on real repo state and assert it returns a valid
...
def test_learn_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test learn", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_learn_runs_and_returns_contract()
