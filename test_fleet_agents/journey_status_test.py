"""journey_status_test — per-agent verifier: run `journey-status` on real repo state and assert it returns a valid
...
def test_journey_status_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test journey-status", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_journey_status_runs_and_returns_contract()
