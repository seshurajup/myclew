"""orchestrate_test — per-agent verifier: run `orchestrate` on real repo state and assert it returns a valid
...
def test_orchestrate_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test orchestrate", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_orchestrate_runs_and_returns_contract()
