"""pipeline_run_test — per-agent verifier: run `pipeline-run` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_pipeline_run_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test pipeline-run", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_pipeline_run_runs_and_returns_contract()
