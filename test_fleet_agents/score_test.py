"""score_test — per-agent verifier: run `score` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_score_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test score", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_score_runs_and_returns_contract()
