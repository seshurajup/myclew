"""ablate_best_test — per-agent verifier: run `ablate-best` on real repo state and assert it returns a valid
...
def test_ablate_best_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test ablate-best", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_ablate_best_runs_and_returns_contract()
