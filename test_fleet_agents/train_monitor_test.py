"""train_monitor_test — per-agent verifier: run `train-monitor` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_train_monitor_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test train-monitor", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_train_monitor_runs_and_returns_contract()
