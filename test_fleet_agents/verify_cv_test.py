"""verify_cv_test — per-agent verifier: run `verify-cv` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_verify_cv_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test verify-cv", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_verify_cv_runs_and_returns_contract()
