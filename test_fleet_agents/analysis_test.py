"""analysis_test — per-agent verifier: run `analysis` on real repo state and assert it returns a valid
...
def test_analysis_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test analysis", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_analysis_runs_and_returns_contract()
