"""arch_probe_test — per-agent verifier: run `arch-probe` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_arch_probe_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test arch-probe", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_arch_probe_runs_and_returns_contract()
