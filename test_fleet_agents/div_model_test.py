"""div_model_test — per-agent verifier: run `div-model` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_div_model_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test div-model", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_div_model_runs_and_returns_contract()
