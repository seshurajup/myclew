"""submission_build_test — per-agent verifier: run `submission-build` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_submission_build_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test submission-build", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_submission_build_runs_and_returns_contract()
