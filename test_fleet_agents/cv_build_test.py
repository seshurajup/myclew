"""cv_build_test — per-agent verifier: run `cv-build` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_cv_build_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test cv-build", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_cv_build_runs_and_returns_contract()
