"""block_synth_test — per-agent verifier: run `block-synth` on real repo state and assert it returns a valid
...
def test_block_synth_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test block-synth", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_block_synth_runs_and_returns_contract()
