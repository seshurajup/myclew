"""config_gen_test — per-agent verifier: run `config-gen` on real repo state and assert it returns a valid
...
def test_config_gen_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test config-gen", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_config_gen_runs_and_returns_contract()
