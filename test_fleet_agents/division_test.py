"""division_test — per-agent verifier: run `division` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_division_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test division", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_division_runs_and_returns_contract()
