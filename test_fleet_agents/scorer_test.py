"""scorer_test — per-agent verifier: run `scorer` on real repo state and assert it returns a valid
...
def test_scorer_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test scorer", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_scorer_runs_and_returns_contract()
