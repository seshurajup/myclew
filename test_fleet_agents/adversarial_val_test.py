"""adversarial_val_test — per-agent verifier: run `adversarial-val` on real repo state and assert it returns a valid
...
def test_adversarial_val_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test adversarial-val", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_adversarial_val_runs_and_returns_contract()
