"""stage_1_div_test — per-agent verifier: run `stage-1-div` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_stage_1_div_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test stage-1-div", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_stage_1_div_runs_and_returns_contract()
