"""notes_sync_test — per-agent verifier: run `notes-sync` on real repo state and assert it returns a valid
...
def test_notes_sync_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test notes-sync", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_notes_sync_runs_and_returns_contract()
