"""notebook_sync_test — per-agent verifier: run `notebook-sync` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_notebook_sync_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test notebook-sync", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_notebook_sync_runs_and_returns_contract()
