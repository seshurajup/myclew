"""public_config_test — per-agent verifier: run `public-config` on real repo state and assert it returns a valid
...
def test_public_config_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test public-config", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_public_config_runs_and_returns_contract()
