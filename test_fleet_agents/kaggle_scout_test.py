"""kaggle_scout_test — per-agent verifier: run `kaggle-scout` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_kaggle_scout_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test kaggle-scout", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_kaggle_scout_runs_and_returns_contract()
