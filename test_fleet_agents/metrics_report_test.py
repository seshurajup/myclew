"""metrics_report_test — per-agent verifier: run `metrics-report` on real repo state and assert it returns a valid
...
def test_metrics_report_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test metrics-report", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_metrics_report_runs_and_returns_contract()
