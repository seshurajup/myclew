"""eda_stats_test — per-agent verifier: run `eda-stats` on real repo state and assert it returns a valid
...
def test_eda_stats_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test eda-stats", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_eda_stats_runs_and_returns_contract()
