"""best_config_test — per-agent verifier: run `best-config` on real repo state and assert it returns a valid
...
def test_best_config_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test best-config", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_best_config_runs_and_returns_contract()
