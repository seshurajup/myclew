"""smoke_test — per-agent verifier: run `smoke` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_smoke_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test smoke", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_smoke_runs_and_returns_contract()
