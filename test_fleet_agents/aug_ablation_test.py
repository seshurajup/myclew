"""aug_ablation_test — per-agent verifier: run `aug-ablation` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
...
def test_aug_ablation_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test aug-ablation", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_aug_ablation_runs_and_returns_contract()
