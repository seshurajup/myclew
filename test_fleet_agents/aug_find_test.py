"""aug_find_test — per-agent verifier: run `aug-find` on real repo state and assert it returns a valid
...
def test_aug_find_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test aug-find", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_aug_find_runs_and_returns_contract()
