"""combine_winners_test — per-agent verifier: run `combine-winners` on real repo state and assert it returns a valid
...
def test_combine_winners_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test combine-winners", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_combine_winners_runs_and_returns_contract()
