"""post_analysis_test — per-agent verifier: run `post-analysis` on real repo state and assert it returns a valid
...
def test_post_analysis_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test post-analysis", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_post_analysis_runs_and_returns_contract()
