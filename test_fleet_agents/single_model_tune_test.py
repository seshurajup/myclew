"""single_model_tune_test — per-agent verifier: run `single-model-tune` with its external calls STUBBED (train-queue/scorer/CLI) and assert it returns a valid
(status, data, to, message) contract with real output (not a crash). Heavy agent: external subprocess stubbed so its LOGIC is tested offline."""
...
def test_single_model_tune_runs_and_returns_contract():
    ...
    try:
        out = raw({"question": "test single-model-tune", "spec": {}}, "test")
    finally:
        subprocess.run = orig
...
def _run():
    ...
    try:
        r = test_single_model_tune_runs_and_returns_contract()
