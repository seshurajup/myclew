...
def predict(
    ...
    for name in tqdm(test_names, desc="Predicting", disable=not INTERACTIVE):
        ...
        if cfg.use_ilp and graph.num_edges() > 0:
            ...
            n_cand = graph.num_edges()
            with suppress_output():
                graph = solver.solve(graph)
            # observability: confirm ILP engaged (solve internals suppressed above)
            print(f"  [ILP] {name}: {n_cand} candidate edges -> {graph.num_edges()} solution edges",
                  flush=True)
        elif cfg.use_ilp:
            print(f"  [ILP] {name}: SKIPPED (num_edges=0)", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--max-frames", type=int, default=None,
                        help="Cap each video to its first N time frames (SCREEN-V1 cost lever, "
                             "rank-validated at N=24). Default None = full T.")
